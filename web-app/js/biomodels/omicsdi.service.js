...
function convertData(datasetId) {
    ...
    return [
        {
            'score': omicsData["viewsCount"], // viewsCount
            'scale': omicsData["viewsCountScaled"], // viewsCountScaled
            'color': '#2E8B57',
            'label': 'Views'
        },
        {
            'score': omicsData["connectionsCount"], // connectionsCount
            'scale': omicsData["connectionsCountScaled"], // connectionsCountScaled
            'color': '#0099cc',
            'label': 'Connections'
        },
        {
            'score': omicsData["citationsCount"], // citationsCount
            'scale': omicsData["citationsCountScaled"], // citationsCountScaled
            'color': '#FF0000',
            'label': 'Citations'
        },
        {
            'score': omicsData["reanalysisCount"], // reanalysisCount
            'scale': omicsData["reanalysisCountScaled"], // reanalysisCountScaled
            'color': '#4B0082',
            'label': 'Reanalyses'
        },
        {
...
function transform(value, fractionSize) {
    if (value == null) {
        return null;
    }
    if (value === 0) {
...
function normalise_number_leaves(data, max_leaves) {
...
function svg_linear_gradient_direction(angle) {
    if (angle < 45) {
        return {x: 50 * Math.tan(angle) + 50, y: 0};
    } else if (angle < 90) {
        return {x: 100, y: 50 * Math.tan(90 - angle)};
    } else if (angle < 135) {
        return {x: 100, y: 50 * Math.tan(135 - angle) + 50};
    } else if (angle < 180) {
        return {x: 50 * Math.tan(180 - angle) + 50, y: 100};
    } else if (angle < 225) {
        return {x: 50 * Math.tan(225 - angle), y: 100};
    } else if (angle < 270) {
        return {x: 0, y: 50 * Math.tan(270 - angle) + 50};
    } else if (angle <= 315) {
        return {x: 0, y: 50 * Math.tan(315 - angle)};
    } else if (angle < 360) {
        return {x: 50 * Math.tan(angle), y: 0};
    }
}
...
function createRosette(datasetId) {
    ...
    const svg = d3.select("#rosette-holder")
        .append('a')
        .style('border-bottom', 'none')
        .attr('href', 'https://www.omicsdi.org/dataset/biomodels/' + datasetId)
        .attr('target', '_blank')
        .attr('title', _title)
        .append('svg')
        .attr('width', this.width)
        .attr('viewBox', '0 0 720 720')
        .attr('class', 'rosette-chart')
        .attr('height', this.width);

    const tool_tip = d3.select("body").append('div')
        .attr('class', 'mt-card tooltip-rosette')
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const gradient = defs.append('linearGradient').attr('id', datasetId + '_gradient_' + i)
    ...
    graph.append('path').attr('d', 'm 378.9895761619361,264.3949755505212 c -5.7581064046051,-9.760471988086' +
        ...
        .attr('fill', 'url(\'' + '#' + datasetId + '_gradient_' + color_index++ + '\')')
