...
class MatlabServiceSpec extends Specification {
    def service
    File tmpFolder = new File("target/testFiles/")

    static doWithSpring = {
        modelFileFormatService(MockFormatService) {
    ...
    def setup() {
        expect:
        service = applicationContext.getBean "matlabService"
        tmpFolder.mkdirs()
    }
...
class MockFormatService {
    ...
    def serviceForFormat(def formatCmd) {
        formatCmd.identifier == 'matlab' ? applicationContext.getBean('matlabService') : null
    }
}
