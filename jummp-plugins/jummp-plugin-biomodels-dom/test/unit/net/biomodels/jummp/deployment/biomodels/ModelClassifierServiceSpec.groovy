...
class ModelClassifierServiceSpec extends Specification {
    ...
    void "test single model classify result parser"() {
        ...
        when:
            ...
            List<JummpEntry<Model, Date>> models = new ArrayList<>()
            models.add(new JummpEntry<Model, Date>(testModel, new Date()))
            Map<?, ?> result = service.classifyModels(models)
        then:
            1 == result.keySet().size()
            JummpEntry<String, String> rootKey = new JummpEntry<>("GO:0008150", "biological_process")
            result.containsKey(rootKey)
            JummpEntry<String, String> parentKey = new JummpEntry<>("GO:0023052", "signaling")
            (result.get(rootKey) as Map).containsKey(parentKey)
            JummpEntry<String, String> classKey = new JummpEntry<>("GO:0007165", "signal transduction")
    ...
    void "test multiple model classify result parser"() {
        given:
            ...
            RestUtils.restTemplate = restTemplate.createMock()
        when:
            List<JummpEntry<Model, Date>> models = responseMock.keySet().collect{
                new JummpEntry<>(createModel(it), new Date())}
            Map<?, ?> result = service.classifyModels(models)
        then:
            1 == result.keySet().size()
            JummpEntry<String, String> rootKey = new JummpEntry<>("GO:0008150", "biological_process")
            ...
            List<Model> classified = (((result.get(rootKey) as Map).get(parentKey) as Map).get(classKey) as List)
            2 == classified.size()
            JummpEntry<String, String> thirdParentKey = new JummpEntry<>("GO:0050896", "response to stimulus")
