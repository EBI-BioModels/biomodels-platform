...
class UhlenModelService extends AutoGenModelService {
    ...
    Map getModelCategoryMap(String pattern) {
        ...
        entries.each {
            AutoGeneratedCategory category = it as AutoGeneratedCategory
            TreeSet<AutoGeneratedCategory> categories
            String currentKey = category.categoryName
            if (returned.containsKey(currentKey)) {
                categories = returned.get(currentKey)
                categories.add(category)
            } else {
                categories = new TreeSet<AutoGeneratedCategory>(abcOrderCategoryComparator())
                categories.add(category)
                returned.put(currentKey, categories)
            }
        }
