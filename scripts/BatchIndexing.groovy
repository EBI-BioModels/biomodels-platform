...
target(loadClasses: 'Loads required classes in the Jummp Grails environment') {
    ...
    RevisionAdapter = loadClass("net.biomodels.jummp.core.adapters.RevisionAdapter")
    ...
    IndexingPlan = loadClass("net.biomodels.jummp.indexing.IndexingPlan")
...
target(main: "Puts everything together to import models from a given folder") {
    ...
    List<IndexingPlan> indexingPlan = IndexingPlan.getAll()
    TreeSet<String> modelsPlanedForIndexing = indexingPlan.collect {
        it.model.submissionId
    }
    modelsImported.each {String modelId ->
        if (!modelsPlanedForIndexing.contains(modelId)) {
	        modelsPlanedForIndexing.add(modelId)
	        def revision = modelService.getRevision(modelId)
	        try {
	            searchService.updateIndex(revision)
	            println "OK! for the model ${modelId}"
	        } catch (Exception e) {
	            println "Exception thrown while indexing ${e.getMessage()}"
            } finally {
	            println "Revision ${revision.id} has been indexed."
	        }
        }
    }
 ...
 */
boolean haveSymlinkToUrlFile = { folder, id ->
