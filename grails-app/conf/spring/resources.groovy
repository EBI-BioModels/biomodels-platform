...
beans = {
    ...
    /* Overrides the plugin's bean of the same name (the security filters look it up by name): a session waiting for
     * its OTP is anonymous everywhere except where the user completes the verification. */
    sessionSecurityContextRepository(HttpSessionSecurityContextRepository) {
        def conf = SpringSecurityUtils.securityConfig
        allowSessionCreation = conf.scr.allowSessionCreation
        disableUrlRewriting = conf.scr.disableUrlRewriting
        springSecurityContextKey = conf.scr.springSecurityContextKey
    }

    securityContextRepository(TwoFactorAwareSecurityContextRepository) {
        def conf = SpringSecurityUtils.securityConfig
        wrapped = ref('sessionSecurityContextRepository')
        realIdentityPaths = [conf.apf.filterProcessesUrl, conf.logout.filterProcessesUrl]
    }
